...
from models.funcs import get_masks, mean_list, loss_func_contra, kr_l1_cohe_loss, replace_linear_vit, distillation_loss, set_seed
...
def get_args():
    ...
    parser.add_argument('--model', type=str, choices=['vit','vit_s','deit_s'], required=True)
...
def _modular_training(model, data_loader, alpha, temperature, \
    ...
    return n_correct/total_labels, log_loss, log_loss_ce, log_loss_cohesion, log_loss_coupling, log_loss_l1, log_loss_contra, log_kernel_rate

def modular_training(model, train_loader, test_loader, alpha, lr_mask, lr_model, num_epochs, teacher_model=None):
            ...
            if not is_cal_modular_metrics:
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    log_loss_l1, log_loss_contra, log_kernel_rate = \
                        _modular_training(model, data_loader, alpha, temperature, optim, phase, teacher_model=teacher_model)
            # draw log
...
def main():
    ...
    if model_name == "vit":
        config = ViTConfig(num_labels=class_num)
    ...
    model = modular_training(model, trainloader, testloader,
                             alpha=alpha, lr_mask=lr_mask, 
                             lr_model=lr_model, num_epochs=n_epochs,
