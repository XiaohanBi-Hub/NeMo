...
def get_args():
    ...
    parser.add_argument('--model', type=str, choices=['vgg16', 'resnet18', 'simcnn', 'rescnn', 'resnet34', 'resnet50'], required=True)
...
def cal_modular_metrics(sample_masks, labels):
    ...
    intersection_sum = intersection_sum[torch.triu(torch.ones_like(intersection_sum, device=DEVICE), diagonal=1) == 1]
    ...
    union_sum = union_sum[torch.triu(torch.ones_like(union_sum, device=DEVICE), diagonal=1) == 1]
...
def main():
    ...
    if model_name == 'vgg16':
        ...
        model = st_simcnn_model().to(DEVICE)
    elif model_name == 'rescnn':
        model = st_rescnn_model().to(DEVICE)
    else:
        raise ValueError
...
if __name__ == '__main__':
    ...
    num_workers = 2
    configs = Configs()
    log_path = f'/home/bixh/Documents/MwT_ext/src/St_logs/{model_name}_{dataset_name}/standard_model_lr_{lr_model}_bz_{batch_size}'
