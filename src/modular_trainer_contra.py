...
from models.funcs import freeze_mask, get_masks, mean_list, loss_func_cos, loss_func_contra, kr_l1_cohe_loss,cal_modular_metrics
...
def modular_training(model, train_loader, test_loader, alpha, temperature, lr_mask, lr_model, num_epochs=200):
        ...
        for data_loader, phase in zip([train_loader, test_loader], ['train', 'test']):
            if not is_cal_modular_metrics:
                with torch.set_grad_enabled(phase == 'train'):
                    log_acc, log_loss, log_loss_ce, log_loss_cohesion, log_loss_coupling, \
                    log_loss_l1, log_loss_contra, log_kernel_rate = \
                        _modular_training(model, data_loader, alpha, temperature, optim, phase)
            else:
                if phase == 'train':
                    with torch.set_grad_enabled(phase == 'train'):
                        log_acc, log_loss, log_loss_ce, log_loss_cohesion, log_loss_coupling, \
            ...
            writer.add_scalar(f'{phase}/Loss-Contra', mean_list(log_loss_contra), epoch)

            print(f"LOSS:{mean_list(log_loss)}")
        scheduler.step()
    return model


def _modular_training(model, data_loader, alpha, temperature, optim, phase):
    if phase == 'train':
        model.train()
    else:
        model.eval()
    ...
    for inputs, labels in tqdm(data_loader, ncols=80):
        ...
        loss_cohesion, loss_coupling, kernel_rate, l1_contra = kr_l1_cohe_loss(masks, labels, DEVICE)
        loss_contra = loss_func_contra(masks, labels, temperature, DEVICE)
        loss = loss_ce + alpha * loss_contra
        ...
        log_loss_contra.append(loss_contra)
    ...
    return n_correct/total_labels, log_loss, log_loss_ce, log_loss_cohesion, log_loss_coupling, \
            log_loss_l1, log_loss_contra, log_kernel_rate
...
def _test_with_modular_metrics(model, test_dataset):
    ...
    for inputs, labels in tqdm(test_dataset, ncols=80):
        ...
        beta=0
        loss_cohesion, loss_coupling, loss_l1, kernel_rate = loss_func_cos(masks, labels, DEVICE=DEVICE)
...
def main():
    ...
    if model_name == 'vgg16':
        model = vgg16(pretrained=False, num_classes=class_num, kn_zoom=1).to(DEVICE)
    ...
    # freeze_mask(_model,1)
    model = model.to(DEVICE)
    print(model)

    model = modular_training(model, train_loader, test_loader, alpha=alpha, temperature=temperature,\
                            lr_mask=lr_mask, lr_model=lr_model, num_epochs=n_epochs)
