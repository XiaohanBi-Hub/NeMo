...
from modules_arch.vit_module_HF import vit_module, calculate_param
...
def get_args():
    ...
    parser.add_argument('--model', type=str, choices=['vit','vit_s'], required=True)
...
def generate_all_modules_masks():
    if model_name == 'vit':
        ...
        mt_model = ViTForImageClassification._from_config(config)
        ...
        mt_model = ViTForImageClassification._from_config(config).to(DEVICE)
    else:
        raise ValueError
    replace_linear_vit(mt_model, replace_attention=True)
    mt_model.classifier = torch.nn.Linear(mt_model.classifier.in_features, 10)
    mt_model = mt_model.to(DEVICE)
    mt_model.load_state_dict(torch.load(mt_model_save_path, map_location=DEVICE),strict=False)
...
def generate_target_module(target_classes, module_mask_path):
    ...
    if model_name == 'vit_s':
        module = vit_module(mt_model_param, target_module_mask, target_classes).to(DEVICE)
        print(module)
    else:
        raise ValueError
...
def main():
    ...
    if model_name == 'vit_s':
        ...
        st_model = ViTForImageClassification._from_config(config).to(DEVICE)
    else:
        raise ValueError
...
if __name__ == '__main__':
    ...
    if contra:
        ...
        mt_model_save_path = f'{save_dir}/lr_{lr_model}_{lr_mask}_a{alpha}_t{temperature}_bs_{batch_size}_noEnc0.pth'
        modules_masks_save_path = f'{save_dir}/lr_{lr_model}_{lr_mask}_a{alpha}_t{temperature}_bs_{batch_size}_noEnc0/mask_thres_{THRESHOLD}.pth'
        
        tc_str = ''.join([str(tc) for tc in target_classes])
        modules_save_path = f'{save_dir}/modules/' \
                            f'lr_model_mask_{lr_model}_{lr_mask}_a_{alpha}_t_{temperature}_bz_{batch_size}_noEnc0/module_tc_{tc_str}_thres_{THRESHOLD}.pth'
    else:
        save_dir = f'{configs.data_dir}/modular_trained/{model_name}_{dataset_name}'

        mt_model_save_path = f'{save_dir}/lr_{lr_model}_{lr_mask}_a{alpha}_b{beta}_bs_{batch_size}_noEnc0.pth'
        modules_masks_save_path = f'{save_dir}/lr_{lr_model}_{lr_mask}_a{alpha}_b{beta}_bs_{batch_size}_noEnc0/mask_thres_{THRESHOLD}.pth'
        
        tc_str = ''.join([str(tc) for tc in target_classes])
        modules_save_path = f'{save_dir}/modules/' \
                            f'lr_model_mask_{lr_model}_{lr_mask}_a_{alpha}_b_{beta}_bz_{batch_size}_noEnc0/module_tc_{tc_str}_thres_{THRESHOLD}.pth'
