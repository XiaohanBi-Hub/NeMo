...
from models.vgg_masked_contra import cifar10_vgg16_bn as mt_vgg16_model
...
from models.resnet_masked import ResNet34 as mt_ResNet34_model
from modules_arch.resnet_module_v2 import ResNet34 as ResNet34_module
from models.resnet import ResNet34 as st_ResNet34_model

from models.resnet_masked import ResNet50 as mt_ResNet50_model
from modules_arch.resnet_module_v2 import ResNet50 as ResNet50_module
from models.resnet import ResNet50 as st_ResNet50_model
...
def get_args():
    ...
    parser.add_argument('--kn_zoom', type=float, default=1.0) 
            ...
            while True:
                ...
                if torch.sum(layer_mask) == 0.0:
                    ...
                    if cur_thres <= 0:
                        ...
                        layer_mask_size = layer_mask.size(0)
                        num_ones = int(0.05 * layer_mask_size)
                        layer_mask[torch.randperm(layer_mask_size)[:num_ones].to(layer_mask.device)] = 1
                        break
                else:
                    break
...
def generate_all_modules_masks():
    if model_name == 'vgg16':
        mt_model = mt_vgg16_model(pretrained=False, kn_zoom=kn_zoom).to(DEVICE)
    elif model_name == 'resnet18':
        mt_model = mt_ResNet18_model(num_classes=10).to(DEVICE)
    elif model_name == 'resnet34':
        mt_model = mt_ResNet34_model(num_classes=10).to(DEVICE)
    elif model_name == 'resnet50':
        mt_model = mt_ResNet50_model(num_classes=10).to(DEVICE)
...
def generate_target_module(target_classes, module_mask_path):
    ...
    if model_name == 'vgg16':
        ...
        module = ResNet34_module(model_param=mt_model_param, module_mask=target_module_mask,
                                 keep_generator=False).to(DEVICE)
    elif model_name == 'resnet50':
        module = ResNet50_module(model_param=mt_model_param, module_mask=target_module_mask,
                                 keep_generator=False).to(DEVICE)
    elif model_name == 'simcnn':
...
def main():
    ...
    if model_name == 'vgg16':
        ...
        st_model = st_ResNet34_model().to(DEVICE)
    elif model_name == 'resnet50':
        st_model = st_ResNet50_model().to(DEVICE)
    elif model_name == 'simcnn':
...
if __name__ == '__main__':
    ...
    log_path = f'{configs.tensorboard_dir}/lr_model_mask_{lr_model}_{lr_mask}_a_{alpha}_bz_{batch_size}'
    if kn_zoom == 1.0:
        save_dir = f'{configs.data_dir}/modular_trained/{model_name}_{dataset_name}'
    else:
        save_dir = f'{configs.data_dir}/modular_trained/{model_name}_{kn_zoom}_{dataset_name}'
    ...
    modules_save_path = f'{save_dir}/modules/' \
                        f'lr_model_mask_{lr_model}_{lr_mask}_a_{alpha}_t{temperature}_bz_{batch_size}/module_tc_{tc_str}_thres_{THRESHOLD}.pth'
