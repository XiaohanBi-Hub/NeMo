...
from models.utils_v2 import MaskConvBNContra
...
def make_layers(cfg: List[Union[str, int]], kn_zoom: float, batch_norm: bool = False) -> nn.Sequential:
    ...
    for v in cfg:
        if v == 'M':
            ...
            if in_channels == 3:
                ...
                mask_conv_bn = MaskConvBNContra(in_channels=in_channels, out_channels=v, kernel_size=3, padding=1)
...
def _vgg(arch: str, cfg: str, batch_norm: bool,
    ...
    model = VGG(make_layers(cfgs[cfg], kn_zoom=kn_zoom, batch_norm=batch_norm), kn_zoom=kn_zoom, **kwargs)
    if pretrained:
