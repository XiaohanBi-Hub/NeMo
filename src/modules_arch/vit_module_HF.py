...
from transformers.models.vit.modeling_vit import ViTSelfAttention, ViTLayer, ViTAttention, ViTIntermediate, ViTOutput
from typing import Optional, Tuple, Union
import copy
# from ..models.funcs import replace_linear_vit

# The ViT module V1 padding the residual operation for alignment
...
class ViTLayerMask(nn.Module):
    """This corresponds to the Block class in the timm implementation."""

    def __init__(self, config: ViTConfig, each_layer_mask) -> None:
        super().__init__()
...
class ViTSelfAttentionMask(nn.Module):
...
def get_vit_module(mt_model, module_mask, vit_config, mask_attention_list, i = 0, att_i = 0, no_mask = 0):
    ...
    for name, child in mt_model.named_children():
        if name in ['layernorm']:
            setattr(mt_model, name, nn.LayerNorm(((module_mask[-1]!=0).int().sum(),), eps=vit_config.layer_norm_eps))
        if isinstance(child, torch.nn.Linear): 
            if no_mask>2:
                no_mask-=1
                continue
            ...
            if name in ['layernorm_before']:
                ...
                layernorm_before = nn.LayerNorm((index_in.sum(),), eps=vit_config.layer_norm_eps)
                setattr(mt_model, name, layernorm_before)
            elif name in ['layernorm_after']:
                index_out = (module_mask[i-3]> 0).int()
                layernorm_after = nn.LayerNorm((index_out.sum(),), eps=vit_config.layer_norm_eps)
                setattr(mt_model, name, layernorm_after)
        else:
            mask_attention_list, i, att_i, no_mask = get_vit_module(child, module_mask, vit_config, mask_attention_list, i, att_i, no_mask)
    return mask_attention_list, i, att_i, no_mask

def replace_vitatt(mt_model, mask_attention_list, no_mask=0, att_i=0):
    # replace ViTSelfAttention with ViTSelfAttentionMask, which has masked qkv and padding algorithm
    # First no_mask selfattention should not be replaced
    for name, child in mt_model.named_children():
        if isinstance(child, ViTSelfAttention):
            if no_mask>0:
                no_mask-=1
                continue
            ...
            att_i, no_mask= replace_vitatt(child, mask_attention_list, no_mask, att_i)
    return att_i, no_mask

def replace_vitlayer(mt_model, each_layer_mask, config, no_mask=0, count=0):
    for name, child in mt_model.named_children():
        if isinstance(child, ViTLayer):
            if no_mask>0:
                no_mask-=1
                continue
            class ViTLayerMask(ViTLayer):
                def __init__(self, config: ViTConfig, layer_mask) -> None:
                ...
                def forward(
                    ...
                    # hidden_states = attention_output + hidden_states[:,:,self.layer_mask[3]!=0]
            ...
            count += 1
            setattr(mt_model, name, ViTLayerMask(config, layer_mask))
        else:
            replace_vitlayer(child, each_layer_mask, config, no_mask, count)

def vit_module(model_param, module_mask, target_classes):
    pic_size=32
    ...
    mt_model = ViTForImageClassification._from_config(vit_config)
    ...
    for size in linear_size_each_encoder:
        ...
        start = end
    mask_attention_list = [ ViTSelfAttentionMask(vit_config) for j in range(12) ]
    # 6 for linear and 2 for layernorm
    mask_attention_list, _, _, _= get_vit_module(mt_model, each_layer_mask, vit_config, mask_attention_list, no_mask = 6*no_mask+2)
    _, _ = replace_vitatt(mt_model, mask_attention_list, no_mask)
    replace_vitlayer(mt_model, each_layer_mask, vit_config, no_mask)
...
if __name__ == "__main__":
    ...
    model_name = 'vit_s'
    mt_model_save_path = '/home/bixh/Documents/MwT_ext/data/data/modular_trained/vit_s_cifar10/lr_0.05_0.05_a0.5_b0.85_bs_128_noEnc0.pth'
    mt_model_param = torch.load(mt_model_save_path, map_location="cuda:0")
    # load modules' masks and the pretrained model
    modules_masks_save_path = f'/home/bixh/Documents/MwT_ext/data/data/modular_trained/vit_s_cifar10/lr_0.05_0.05_a0.5_b0.85_bs_128_noEnc0/mask_thres_0.9.pth'
