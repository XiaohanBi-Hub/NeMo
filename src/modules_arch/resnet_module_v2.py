...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
        ...
        self.conv1 = mask_conv1x1_bn(inplanes, width)
        # self.bn1 = norm_layer(width)
        self.conv2 = mask_conv3x3_bn(width, width, stride, groups, dilation)
        # self.bn2 = norm_layer(width)
        self.conv3 = mask_conv1x1_bn(width, planes * self.expansion)
...
def ResNet34(model_param, module_mask, keep_generator=True, **kwargs):
    from models.resnet_masked import ResNet34 as mt_ResNet34_Model
    mt_model = mt_ResNet34_Model()
    detailed_masks = get_detailed_masks(mt_model, module_mask)

    module = _resnet(BasicBlock, [3, 4, 6, 3], detailed_masks=detailed_masks, keep_generator=keep_generator, **kwargs)
    module_param = get_module_param(model_param, detailed_masks, keep_generator=keep_generator)
    module.load_state_dict(module_param)
    return module
...
def ResNet50(model_param, module_mask, keep_generator=True, **kwargs):
    from models.resnet_masked import ResNet50 as mt_ResNet50_Model
    mt_model = mt_ResNet50_Model()
    detailed_masks = get_detailed_masks(mt_model, module_mask)

    module = _resnet(Bottleneck, [3, 4, 6, 3], detailed_masks=detailed_masks, keep_generator=keep_generator, **kwargs)
    module_param = get_module_param(model_param, detailed_masks, keep_generator=keep_generator)
    module.load_state_dict(module_param)
    return module
